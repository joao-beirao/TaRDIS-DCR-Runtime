...
import protocols.dcr.messages.OnTxMessage;
...
public class DistributedDCRProtocol
    ...
    private static final Logger logger = LogManager.getLogger(DistributedDCRProtocol.class); // logger for the protocol

    private int channelId; // id of the channel used by the protocol

    private int nextPingId = 0; // id of the next ping message to send

    // Ping targets of all nodes
    ...
    public void init(Properties props) throws IOException, HandlerRegistrationException {
        ...
        //         break;
        //     default:
        //         logger.warn("Node name not recognized");
        //         break;
        // }
        ...
        registerMessageSerializer(channelId, OnTxMessage.MSG_ID, OnTxMessage.serializer);
    ...
    private void uponReceivePingRequest(AppRequest appRequest, short sourceProtocol) {
        logger.info("Received dcr request to ping {} with message {} and extension {}",
                appRequest.getDestination(), appRequest.getEventId(), appRequest.getIdExtensionToken());
    ...
    public void uponReceivePingMessage(OnTxMessage msg, Host from, short sourceProto, int channelId) {
        ...
        AppRequest request = new AppRequest(msg.getMessage(), msg.getMarking(), from, msg.getSender(),
                msg.getIdExtensionToken());
