...
import dcr1.common.data.values.Undefined;
...
import dcr1.runtime.events.LocallyInitiatedEventInstance;
import dcr1.runtime.events.RemotelyInitiatedEventInstance;
...
//
    ...
    private void assertAdmissibleValueType(GenericEventInstance<?> event, Value<?> replaceValue) {
        ...
        if (!event.getValue().type().equals(replaceValue.type())) {
    ...
    private void instantiateEventElement(EventElement<?> baseElement, String idExtension,
            SpawnContext spawnContext) {
        if (!canInstantiate(baseElement.constraint(), spawnContext.evalEnv)) {
            System.err.println("Unable to instantiate " + baseElement.getElementId());
            return;
        } else
            System.err.println("Instantiatng " + baseElement.getElementId());
        ...
        Value<?> instanceValue = instance.marking().getValue();

        // TODO use bindIfAbsent above
        eventInfo.valueEnv()
                .bindIfAbsent(instance.localId(), instanceValue.wrap(baseElement.label()));
        ...
        if (!spawnRelations.isEmpty()) {
            ...
            spawnRelations.values()
                    .forEach(list -> list.stream()
                            .map(info -> "\n" + info.spawn.getBaseElement().getSourceId() +
    ...
    record SpawnContext(Environment<Value<?>> evalEnv,
        ...
        SpawnContext beginScope(EventInstance<?> triggerEvent, UserVal triggerCounterpart) {
            var newEvalEnv = evalEnv.beginScope("@trigger",
                    triggerEvent.getValue().wrap(triggerEvent.label()));
