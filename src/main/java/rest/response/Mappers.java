    ...
    public static TypeDTO fromType(Type type) {
        return switch (type) {
            ...
            case RecordType ty -> new RecordTypeDTO(ty.fields().stream()
                    .collect(Collectors.toMap(Record.Field::name,
                            f -> fromType(f.value()))));
    ...
    public static EventDTO toEventDTO(EventInstance e) {
        ...
        return switch (e) {
            case ComputationEventInstance e1 ->
                    EventDTO.newComputationEventDTO(id, typeExpr, e1.receivers()
                                    .isPresent() ? KindDTO.COMPUTATION_SEND :
                                    KindDTO.COMPUTATION,
                            marking);
            case InputEventInstance e2 ->
                    EventDTO.newInputEventDTO(id, typeExpr, e2.receivers()
